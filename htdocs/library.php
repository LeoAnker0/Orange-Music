...
function getSmallerIndex($uuid, $current_index) {
    ...
    $sql = "SELECT libraryIndex FROM library WHERE user_added = ? AND libraryIndex < ? ORDER BY ABS(libraryIndex - ?) ASC LIMIT 1";


    // Prepare the statement and bind the parameters
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sii", $uuid, $current_index, $current_index);
...
$logfile = "/Applications/XAMPP/xamppfiles/htdocs/logfile.log";
//$message = "json as received: " . $json;
//error_log($message . "\n", 3, $logfile);
//$message = "libraryIndex and uuid as received: " . $uuid . " : " . $libraryIndex;
//error_log($message . "\n", 3, $logfile);
...
if ($libraryIndex == 9999999999999) {
  $libraryIndex = getSmallerIndex($uuid, $libraryIndex);
}
