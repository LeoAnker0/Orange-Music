...
*/
async function getValueFromDatabase(uuid) {
  // Make a request to the server to retrieve the value
    //console.warn("get value from getValueFromDatabase")

  const response = await fetch('getNoItemsInLibrary.php', {
    method: 'POST',
...
async function getLibraryItemFromDatabase(libraryIndex, uuid) {
    // Make a request to the server to retrieve the value
    ...
    const response = await fetch('library.php', {
        method: 'POST',
        body: JSON.stringify({ libraryIndex: libraryIndex, uuid: uuid}),
        headers: { 'Content-Type': 'application/json' }
    });
...
async function getItemsInLibrary() {
  ...
  const number = await getValueFromDatabase(artistNameUUID);
...
async function getLibraryItems(libraryIndex, artistNameUUID) {
    const allLibraryItems = await getLibraryItemFromDatabase(libraryIndex, artistNameUUID);
    return allLibraryItems;
}


function buildMusicObject(CounterStart) {
    ...
    async function awaitForMusicObjectReturn() {
        ...
        const libraryItemArray = await getLibraryItemFromDatabase(libraryIndex, artistNameUUID);
        ...
        var howToHandle = "TYPEalbumEPsingle" //the 2 types that a music object can support are: TYPEalbumEPsingle; TYPEplaylist
...
function buildLibrary() {
    ...
    //change the routing to the proper name
    ...
    buildMusicObject(LoadedMusicObjects)
